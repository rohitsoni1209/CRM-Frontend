...
export const QuotesDetail = (props) => {
  ...
  const detail = useSelector((state) => state.Quotes.Detail);
  const { id } = useParams();
  ...
  useEffect(() => {
    dispatch(GET_USER_PROFILE()).then((res) => {
      if (res?.success) {
        formik.setFieldValue("QuoteOwnerId", res?.data?.data[0]?.userId);
      }
    });
  }, []);
  // useEffect(() => {
...
// import useAccessibleRole from "../../../hooks/useAccessibleRole";
// export const QuotesDetail = (props) => {
//   const data = useSelector((state) => state.Quotes.Detail);
//   const { id } = useParams();
//   const navigate = useNavigate();
//   const dispatch = useDispatch();
//   const [editable, setEditable] = useState(true);
...
//             values.orderedItems = formik.values.orderedItems;
//             dispatch(
//               UPDATE__DATA_SaleOrder({ ...values, ModuleTitle: "quotes" })
//             ).then(() => {
//               navigate("/crm/quotes");
//             });
//           }}
