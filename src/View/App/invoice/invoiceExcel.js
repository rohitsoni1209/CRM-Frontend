...
const InvoiceExcel = () => {
  ...
  console.log("sections", sections,filters);
    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        ...
        const res = await dispatch(
          IMPORT_BULK_EXCEL("add-invoices-excel", excelData)
        );
  ...
  useEffect(() => {
    console.log("filters===>123", filters);
    //if (data?.length > 0 && filters) {
    var headercollect = [];
    if (filters.length > 0) {
      for (var i = 0; i < filters.length; i++) {
        //console.log(filters[i]);
        headercollect.push(filters[i]);
      }

      // console.log("headercollect", headercollect);
