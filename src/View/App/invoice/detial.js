...
import { CREATE__DATA_SaleOrder } from "../../../Redux/actions/invoices";
...
export const InvoicesDetail = (props) => {

  const navigate = useNavigate();
  const dispatch = useDispatch();
  // const { formType } = props;
  const { formType = "Invoices" } = props;

  const api = list["Invoice"] || {};
  const { edit, write } = useAccessibleRole("Invoices");
  ...
  const detail = useSelector((state) => state.Invoices.Detail);
  const { id } = useParams();
  ...
  const generateYupSchema = (sections, detail) => {
    ...
    sections.map((section, sectionTitle) => {
      if (Array.isArray(section?.inputs)) {
        ...
        //   console.log("section?.inputs---->", section?.inputs[index], detail.orderedItems, TableHeadingObj);
  ...
  useEffect(() => {
    dispatch(GET_USER_PROFILE()).then((res) => {
      if (res?.success) {
        formik.setFieldValue("InvoicesOwnerId", res?.data?.data[0]?.userId);
      }
    });
  }, []);
  // useEffect(() => {
            ...
            <div className="form-group1" key={input?.lookupModule[0]}>
              ...
              <div className="mt-2 widthfull" style={{ display: "flex" }}>
                <LookupModuleField
                  ...
                  labelType={input.lookupModule && (input.lookupModule).slice(-1) == "s" ? input?.lookupModule : input?.lookupModule ? input?.lookupModule + "s" :
                    moduleName
                      ? moduleName
                      : input?.label?.toLowerCase().includes("related")
                        ? "related"
                                            ...
                                            return (
                                              ...
                                              >
                                                <LookupModuleField
                                                  ...
                                                  labelType={input.lookupModule && (input.lookupModule).slice(-1) == "s" ? input?.lookupModule : input?.lookupModule == "inventory" ? input?.lookupModule : input?.lookupModule ? input?.lookupModule + "s" :
                                                    moduleName
                                                      ? moduleName
                                                      : input?.label?.toLowerCase().includes("related")
                                                        ? "related"
...
// export const InvoicesDetail = (props) => {
//   const data = useSelector((state) => state.Invoices.Detail);
//   const { id } = useParams();
//   const dispatch = useDispatch();
//   const editable = true;
...
//             values.orderedItems = formik.values.orderedItems;
//             dispatch(
//               UPDATE__DATA_SaleOrder({ ...values, ModuleTitle: "invoices" })
//             );
//           }}
//         >
//           {({ isSubmitting }) => (
