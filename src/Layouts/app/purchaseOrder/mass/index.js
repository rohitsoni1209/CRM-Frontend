...
const MassPurchaseOrderModules = ({ location }) => {
  ...
  const {
    ...
    write,
    delete: deleteValue,
  } = useAccessibleRole("purchaseorders");
  ...
  const handleMassDelete = () => {
    if (checkRows?.length !== 0) {
      dispatch(
        MASS_DELETE("purchase-orders-mass-delete", { dataList: [...checkRows] })
      ).then((res) => {
        if (res?.status === 200) {
          ClearFilter();
  ...
  const handleMassUpdate = () => {
    if (newInput?.field && newInput?.value) {
      dispatch(
        MASS_UPDATE("purchase-orders-mass-update", {
  ...
  const handleMassTransfer = () => {
    if (checkRows?.length !== 0) {
      dispatch(
        MASS_TRANSFER("purchase-orders-mass-transfer", {
    ...
    <div className="flex gap-3   flex-col w-full">
      <div className="bg-white p-10 rounded-2xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <input
            type="text"
            className="bg-[#F9F9FB] rounded-xl px-4 py-3 focus:outline-none w-5/12"
...
export default MassPurchaseOrderModules;
