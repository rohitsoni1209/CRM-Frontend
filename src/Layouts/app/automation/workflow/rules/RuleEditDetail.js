...
const RuleDetailsEdit = ({ ruleId }) => {
  ...
  const { workflowIdData } = useSelector((store) => store?.workflowReducer);
  ...
  const removeCondition = (index) => {
    ...
    setConditionArray(array?.filter((ele, i) => i !== index));
  };
  ...
  const handlePost = async () => {
    // if (schedule?.count && schedule?.time && actionScheduledData?.length > 0) {
    let newValue = {
      WorkFlowsOwnerId: userId,
      status: true,
      ModuleTitle: workflowIdData?.ModuleTitle,
      RuleName: workflowIdData?.RuleName,
      Description: desc,
      when: {
    ...
    const res = await dispatch(UPDATE_WORKFLOW(ruleId, newValue));
    if (res?.success) {
  ...
  useEffect(() => {
    if (workflowIdData?.ModuleTitle) {
      dispatch(GET_FILTERS(list[workflowIdData?.ModuleTitle]?.filterApi)).then(
        (res) => {
          if (res) {
  ...
  useEffect(() => {
    if (workflowIdData) {
      setDesc(workflowIdData?.Description);
      setWhen({
        selected: true,
        repeat: workflowIdData?.when?.Repeat,
        value: workflowIdData?.when?.ExecuteAction?.create
          ? workflowIdData?.when?.ExecuteAction?.edit
            ? "Create or Edit"
            : "Create"
          : workflowIdData?.when?.ExecuteAction?.edit
          ? "Edit"
      ...
      setConditionShow(!workflowIdData?.condition?.all);
      let array = [];
      for (const [key, value] of Object.entries(
        workflowIdData?.condition?.certain
      )) {
      ...
      if (workflowIdData?.InstantActions?.length) {
        let assignDataObj = {};
        let assignInstant = workflowIdData?.InstantActions?.filter(
          (ele) => ele?.ActionName === "assignowner"
        );
        if (assignInstant?.length) {
          assignDataObj = {
            ...assignDataObj,
            instantAction: assignInstant[0]?.data?.OwnerId,
          };
        }
        let assignScheduled = workflowIdData?.scheduledActions?.action?.filter(
          (ele) => ele?.ActionName === "assignowner"
        );
        if (assignScheduled?.length) {
          assignDataObj = {
            ...assignDataObj,
            scheduledAction: assignScheduled[0]?.data?.OwnerId,
          };
        }
        setAssignData(assignDataObj);

        setActionData(
          workflowIdData?.InstantActions?.filter(
            (ele) =>
              ele?.ActionName !== "assignowner" &&
              ele?.ActionName !== "fieldupdate"
          )
        );
      }
      if (workflowIdData?.InstantActions?.length) {
        let fieldDataObj = {};
        let assignInstant = workflowIdData?.InstantActions?.filter(
          (ele) => ele?.ActionName === "fieldupdate"
        );
        if (assignInstant?.length) {
          fieldDataObj = {
            ...fieldDataObj,
            instantAction: {
              name: assignInstant[0]?.Name,
              field: Object.keys(assignInstant[0]?.data)[0],
              data: Object.values(assignInstant[0]?.data)[0],
            },
          };
        }
        let assignScheduled = workflowIdData?.scheduledActions?.action?.filter(
        ...
        if (assignScheduled?.length) {
          fieldDataObj = {
            ...fieldDataObj,
            scheduledAction: {
              name: assignScheduled[0]?.Name,
              field: Object.keys(assignScheduled[0]?.data)[0],
              data: Object.values(assignScheduled[0]?.data)[0],
            },
          };
        }
        setFieldData(fieldDataObj);
      }
      if (workflowIdData?.scheduledActions?.action?.length) {
        setActionScheduledData(
          workflowIdData?.scheduledActions?.action?.filter(
            (ele) =>
              ele?.ActionName !== "fieldupdate" &&
              ele?.ActionName !== "assignowner"
          )
        ...
        setSchedule({
          count: workflowIdData?.scheduledActions?.time?.count,
          time: workflowIdData?.scheduledActions?.time?.time,
        });
      }
    }
  }, [workflowIdData]);
  ...
  useEffect(() => {
    if (ruleId) {
      dispatch(GET_WORKFLOW_BY_ID(ruleId));
    }
  }, [ruleId]);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h6 className="text-[#191242] text-xl font-semibold ">
                {workflowIdData?.RuleName} - @{workflowIdData?.ModuleTitle}
              </h6>
              <LockIcon />
            </div>
          </div>
                  ...
                  <div>
                    <div className="flex items-start ">
                      ...
                      <div className="p-6 bg-[#fff] rounded-lg w-[700px]">
                        <div className="">
                          <p>
                            Which {workflowIdData?.ModuleTitle} would you like
                            ...
                            <div className="flex items-center  gap-[18px] m-3 ">
                              ...
                              <p className="text-[#18181B] text-base font-medium roup-hover:rate  leading-[20px]">
                                <span className="text-[#929296] text-sm font-medium leading-[22px] ml-2">
                                  {workflowIdData?.ModuleTitle} matching certain
                                  conditions
                                </span>
                              </p>
                              <input
                              ...
                              <p className="text-[#18181B] text-base font-medium roup-hover:rate  leading-[20px]">
                                <span className="text-[#929296] text-sm font-medium leading-[22px] ml-2">
                                  All {workflowIdData?.ModuleTitle}
                                </span>
                              </p>
                            </div>
                          </div>
...
export default RuleDetailsEdit;
