...
const MassQuotesModules = ({ location }) => {
  ...
  const { edit, write, delete: deleteValue } = useAccessibleRole("quotes");
  ...
  const handleMassDelete = () => {
    if (checkRows?.length !== 0) {
      dispatch(
        MASS_DELETE("quotes-mass-delete", { dataList: [...checkRows] })
      ).then((res) => {
        if (res?.status === 200) {
          ClearFilter();
      ...
      toast.warning(`Please select Quotes list list first`);
    }
  };
  ...
  const handleMassUpdate = () => {
    if (newInput?.field && newInput?.value) {
      dispatch(
        MASS_UPDATE("quotes-mass-update", {
  ...
  const handleMassTransfer = () => {
    if (checkRows?.length !== 0) {
      dispatch(
        MASS_TRANSFER("quotes-mass-transfer", {
...
export default MassQuotesModules;
