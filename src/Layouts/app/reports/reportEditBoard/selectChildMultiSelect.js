...
const MultiSelectChildModule = ({ valuehooks, options, index, value }) => {
    const { handleChildModuleSelect } = valuehooks

    return (
        <div className="p-4 border w-96 rounded-md bg-blue-100/40">
            <Listbox value={value?.join()} onChange={(e) => handleChildModuleSelect(e, index)}>
                <div className="relative mt-2 cursor-pointer">
                    <Listbox.Button className="relative w-1/2 rounded-lg bg-yellow-100/50 py-2 pl-3 pr-10 text-left border focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                        <span className="block truncate">{value?.join() || 'Select Module'}</span>
                    </Listbox.Button>
                    <Transition
                        as={Fragment}
                        ...
                        <Listbox.Options className="absolute mt-1 max-h-36 w-1/2 overflow-scroll rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                            {options?.map((item, itemIdx) => (
                                <Listbox.Option
                                    key={itemIdx + item}
                                    className={({ active }) =>
                                        `relative select-none py-2 px-2 ${active || value?.includes(item) ? 'bg-amber-50 text-primary' : 'text-gray-900'
                                        }`
                                    }
                                    value={item}
                                >
                                    <span
                                        className={`block truncate ${value?.includes(item) ? 'font-medium' : 'font-normal'
                                            }`}
                                    >
            ...
            <h1 className="mt-1 text-sm">Select Sub Modules</h1>
        </div>
    )
}

export default MultiSelectChildModule;