...
import SaleUpdateModal from "./saleUpdateModal";
import useAccessibleRole from "../../../../hooks/useAccessibleRole";
import moment from "moment";

const MassSaleOrderDelete = ({ location }) => {
  ...
  const { edit, write, delete: deleteValue } = useAccessibleRole("saleorders");
  ...
  console.log("pagintableHeaderation", pagination);
  // setp 2 for filter
  ...
  const handleMassDelete = () => {
    if (checkRows?.length !== 0) {
      dispatch(
        MASS_DELETE("sale-orders-mass-delete", { dataList: [...checkRows] })
      ).then((res) => {
        if (res?.status === 200) {
          ClearFilter();
      ...
      toast.warning(`Please select Sale-order list first`);
    }
  };
  ...
  const handleMassUpdate = () => {
    if (newInput?.field && newInput?.value) {
      dispatch(
        MASS_UPDATE("sale-orders-mass-update", {
  ...
  const handleMassTransfer = () => {
    if (checkRows?.length !== 0) {
      dispatch(
        MASS_TRANSFER("sale-orders-mass-transfer", {
...
export default MassSaleOrderDelete;
