...
import { GET_TEMPLATE_SMS_LIST, DELETE_TEMPLATE } from "../../../../Redux/actions/serviceControl";
...
const Templates = ({ currentPage, data }) => {
    ...
    useEffect(() => {
        dispatch(GET_TEMPLATE_SMS_LIST({
            offset: currentPage,
            limit: 10,
        }))
    }, [])
                        ...
                        return (
                            <div key={item?._id} className="cursor-pointer mt-2 p-2.5 bg-gray-50 rounded-md shadow-lg hover:shadow-xl hover:bg-white">
                                <div className="flex justify-between items-center">
                                    <div className=" w-3/4 ">
                                        <p className="text-gray-600 text-xl"> {item?.SmsTitle}</p>
                                    </div>
                                    <div className="flex justify-between space-x-2">
                                        <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center">
                                            <DeleteItem item={item}
                                            des={item?.smsDescription}
                                            title={item?.SmsTitle} 
