...
const MassTableList = (props) => {
  ...
  const { delete: deleteValue } = useAccessibleRole(moduleName);
  ...
  const applyFilter = (offset = 1, limit = 10) => {
    ...
    if (isApply) {
      ...
      dispatch(GET_ALL_DATA_FILTER(api.getApi, payload, massTrue));
      setShowRows(10);
    } else {
  ...
  const handleMassDelete = () => {
    if (checkRows?.length !== 0) {
      dispatch(MASS_DELETE(api.deleteUrl, { dataList: [...checkRows] })).then(
        (res) => {
          if (res?.status === 200) {
            ClearFilter();
